let iTrade = (function(){
	...
	function buildTableWinSeries(ano){
		...
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano-1, 11, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 1, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
			//Para segunda parte de Dezembro
			if (today.getMonth() == 11 && today.getDate() >= getQuartas15(ano, 11))
				serie_class = " class='its_me'";
			//Para Janeiro
		...
		html += `<tr><td>${data}</td><td${serie_class}>G</td></tr>`;
		//Fev - Abr
		first_day = getQuartas15(ano, 1);
		last_day = getQuartas15(ano, 3)-1;
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 1, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 3, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
		...
		html += `<tr><td>${data}</td><td${serie_class}>J</td></tr>`;
		//Abr - Jun
		first_day = getQuartas15(ano, 3);
		last_day = getQuartas15(ano, 5)-1;
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 3, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 5, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
		...
		html += `<tr><td>${data}</td><td${serie_class}>M</td></tr>`;
		//Jun - Ago
		first_day = getQuartas15(ano, 5);
		last_day = getQuartas15(ano, 7)-1;
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 5, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 7, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
		...
		html += `<tr><td>${data}</td><td${serie_class}>Q</td></tr>`;
		//Ago - Out
		first_day = getQuartas15(ano, 7);
		last_day = getQuartas15(ano, 9)-1;
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 7, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 9, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
		...
		html += `<tr><td>${data}</td><td${serie_class}>V</td></tr>`;
		//Out - Dez
		first_day = getQuartas15(ano, 9);
		last_day = getQuartas15(ano, 11)-1;
		data = `${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 9, 1)))} (${first_day}) até ${capitalizeFirstLetter(new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, 11, 1)))} (${last_day})`;
		if (today.getFullYear() == ano){
			serie_class = "";
		...
		html += `<tr><td>${data}</td><td${serie_class}>Z</td></tr>`;
	...
	function buildTableWdoSeries(ano){
		...
		for (let m=0; m<12; m++){
			let first_day = (new Date(ano, m, 1)).getDate(),
				...
				month_name = new Intl.DateTimeFormat('pt-BR', {month: 'short'}).format(new Date(ano, m, 1)),
				data = `${capitalizeFirstLetter(month_name)} (${first_day} - ${last_day})`,
				serie_class = '';
			if (today.getFullYear() == ano && today.getMonth() == m)
				serie_class = " class='its_me'";
			html += `<tr><td>${data}</td><td${serie_class}>${series[m]}</td></tr>`;
		}
	...
	function buildSectionOperacoes(){
		let inputs = $(document.getElementById("table_operacoes")),
			html = ``;
		...
		inputs.find("input[name='op']").inputmask({regex: "c|v", placeholder: ""});
	...
	/*------------------------------ Section Operacoes -------------------------------*/
	$("select[name='ativo']", document.getElementById("table_operacoes")).change(function (){
		if (this.options[this.selectedIndex].text === "WIN")
			$(document.getElementById("table_operacoes")).find("input[name='entrada'],input[name='stop'],input[name='men'],input[name='saida'],input[name='mep']").inputmask({mask: "999.999", placeholder: ''});
		else if (this.options[this.selectedIndex].text === "WDO")
			$(document.getElementById("table_operacoes")).find("input[name='entrada'],input[name='stop'],input[name='men'],input[name='saida'],input[name='mep']").inputmask({mask: "9.999", placeholder: ''});
	});
	/*------------------------------- Section Ativos ---------------------------------*/
	...
	$("button", document.getElementById("menu_bottom")).click(function (){
		let me = this.name;
		$("body > section").each(function (i, elem){
			if (elem.id === me){
				if (elem.id === "ativos" && !(elem.id in _builded_sections))
					_builded_sections.push(buildSectionAtivos());
				else if (elem.id === "operacoes" && !(elem.id in _builded_sections))
					_builded_sections.push(buildSectionOperacoes());
				$(elem).show();
			}
			else
				$(elem).hide();
